...
table(train_data2017$cat_route)
table(test_data2018$cat_route)
...
model.svm <- train(svm.learner, 
               trainTask)

parallelStop()

# Importance des variables
imp <- getFeatureImportance(model.svm)
imp <- t(imp$res)
...
# Predictions
pred <- predict(model.svm, newdata = test_data2018)
